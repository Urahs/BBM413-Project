...
class GUI:
  ...
  def apply(self, val, condition):
    ...
    if condition == "brightness":
      ...
      self.edited_image = rotation.rotation(self.temp_image, self.sliderValue, is_grayscale.is_grayscale(self.temp_image))
    elif condition == "blur":
...
● sum of the same coordinate values must be smaller than 100""", font=("Consolas", 12), anchor="e")
  ...
  def is_there_problem(self, special_condition = "none"):
    if self.isLoadedImageEmpty():
      return True
    elif is_grayscale.is_grayscale(self.temp_image) and special_condition in ["color_balance", "saturation"]:
  ...
  def saturation(self):
    if self.is_there_problem("saturation"):
      return
  ...
  def color_balance(self):
    if self.is_there_problem("color_balance"):
      return
    self.clear_old_frames()
